...
(function () {
  ...
  function renderOverlay() {
    ...
    panel.innerHTML = `
      ...
      <div class="actions">
        ...
        ${state.entities.length ? `<button data-act="sync" class="ghost">${synced ? "Sync" : "Connect"}</button>` : ""}
      </div>
  ...
  chrome.storage.sync.get(["autoScan", "apiUrl", "token"], (cfg) => {
    settings = { autoScan: cfg.autoScan !== false, apiUrl: cfg.apiUrl || "", token: cfg.token || "" };
    if (settings.autoScan && document.body) {
