...
(function () {
  ...
  chrome.storage.sync.get(["typeGuard", "blockSend"], (cfg) => {
  ...
  chrome.storage.onChanged.addListener((c) => {
    ...
    if (!settings.typeGuard) clearVisuals();
  });
  ...
  document.addEventListener("input", (e) => {
    if (!settings.typeGuard || !isEditable(e.target)) return;
    active = e.target;
  ...
  document.addEventListener("focusin", (e) => {
    if (settings.typeGuard && isEditable(e.target)) { active = e.target; refresh(active); }
  }, true);
  ...
  function openPop(badge) {
    ...
    const items = lastEnts.slice(0, 12).map((e) =>
      `<div class="it"><span class="t">${label(e.type)}</span><span class="v">${NS.maskPreview(e.text)}</span></div>`
    ).join("");
    const more = lastEnts.length > 12 ? `<div class="more">+ ${lastEnts.length - 12} more…</div>` : "";
    ...
    pop.innerHTML = `
      ...
      ${banner}
      <div class="list">${items}${more}</div>
      <div class="pa"><button data-a="redact">Redact all</button><button data-a="dismiss" class="g">Dismiss</button></div>`;
  ...
  // --- Redaction ---------------------------------------------------------
  function redactAll(el) {
    if (!el) return;
    const ctx = getContext(el);
    const n = NS.detect(ctx.text).length;
    let masked = ctx.text;
    [...NS.detect(ctx.text)].sort((a, b) => b.start - a.start).forEach((e) => {
    ...
    popOpen = false;
    refresh(el);
    if (n) toast("✓ Redacted " + n + " item" + (n === 1 ? "" : "s"), "#22c55e");
  }
  ...
  const CSS = `
    ...
    .list { max-height: 190px; overflow: auto; display: flex; flex-direction: column; gap: 5px; margin-bottom: 9px; }
