...
(function (global) {
  ...
  // The user can switch North Star OFF on specific websites (e.g. youtube.com).
  // Disabled hosts are stored locally as a list of hostnames; matching is
  // subdomain-aware so "youtube.com" also covers "www.youtube.com".
  function normHost(h) {
    return String(h || "")
      .trim().toLowerCase()
      .replace(/^https?:\/\//, "")
  ...
  function hostIsDisabled(host, list) {
    if (!list || !list.length) return false;
    const h = normHost(host);
    if (!h) return false;
    return list.some((raw) => {
      const e = normHost(raw);
  ...
  // Per-rule action: "redact" = auto-mask this value; anything else = "highlight"
  // (warn only, let the user decide). Actions are keyed by the rule text and
  // stored/edited on the local Options page. Nothing about them leaves the device.
  function _actionOf(actions, key) {
    return actions && String(actions[key]).toLowerCase() === "redact" ? "redact" : "highlight";
  }
  ...
  function setCustomRules(rules) {
    ...
    (rules.terms || []).forEach((term) => {
      ...
      FRIENDLY[type] = term;
      dets.push({ type, action: _actionOf(actions, term), conf: 0.95, re: new RegExp("(?<!\\w)" + _escapeRe(term) + "(?!\\w)", "gi") });
    });
    (rules.keywords || []).forEach((kw) => {
      ...
      FRIENDLY[type] = kw;
      dets.push({ type, action: _actionOf(actions, kw), conf: 0.9, group: 1, re: new RegExp("\\b" + _escapeRe(kw) + "\\b\\s*[:=#-]?\\s*(\\S{1,63}[\\w#@])", "gi") });
    });
    (rules.regexes || []).forEach((rx) => {
      ...
      try {
        ...
        dets.push({ type, action: _actionOf(actions, rx), conf: 0.9, re: new RegExp(rx, "gi") });
      } catch (_) { /* ignore invalid regex */ }
    });
    customDetectors = dets;
  }

  // Resolve the effective action for a detected entity. Built-in detectors have
  // no action (they're warn-only — the user redacts manually); only user rules
  // opt into auto-redaction.
  const actionFor = (e) => (e && e.action === "redact" ? "redact" : "highlight");
  ...
  function detect(text) {
    ...
    for (const p of active) {
      ...
      while ((m = p.re.exec(text)) !== null) {
        ...
        ents.push({ type: p.type, text: val, start: idx, end: idx + val.length, confidence: p.conf, action: p.action });
      }
    }
    // Resolve overlaps: for any overlapping region keep the highest-severity
    // finding, so "aadhaar card 4327…" is labelled Aadhaar, not phone. A user
    // rule explicitly marked "redact" wins first (their choice to auto-strip a
    // value beats a built-in that would only highlight it), then we sort by
    // severity (weight × confidence), then longer span, and greedily claim spans.
    const isRedact = (e) => (e.action === "redact" ? 1 : 0);
    ents.sort((a, b) =>
      isRedact(b) - isRedact(a) ||
  ...
  global.NorthStarEngine = {
    ...
    actionFor, normHost, hostIsDisabled, CREDENTIALS, FRIENDLY, WEIGHTS,
  };
  ...
  function _merge(a, b) {
    ...
    // User actions take precedence over managed defaults for the same key.
    const actions = Object.assign({}, a.actions || {}, b.actions || {});
    return {
      terms: u(a.terms, b.terms), keywords: u(a.keywords, b.keywords),
      regexes: u(a.regexes, b.regexes), actions,
    };
  }
  function _loadRules() {
