...
async function siteOff(tab) {
  try {
    const host = tab && tab.url ? new URL(tab.url).hostname : "";
    const cfg = await chrome.storage.sync.get(["disabledSites"]);
    return NS.hostIsDisabled(host, cfg.disabledSites || []);
  } catch (_) { return false; }
}

chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (!tab || tab.id == null) return;
  if (await siteOff(tab)) {
    chrome.notifications.create({
      type: "basic", iconUrl: "../icons/icon128.png",
      title: "North Star is off on this site",
      message: "Turn it back on from the North Star popup to scan here.",
    });
    return;
  }
